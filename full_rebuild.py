...
with open('app_logic_clean.py', 'r') as f:
    orig_code = f.read()

def get_body(source, name):
...
IMPORTS = r'''import tkinter as tk
...
NEW_INIT = r'''    def __init__(self, root):
        self.root = root
...
NEW_SETUP_UI = r'''    def setup_ui(self):
...
V5_LOGIC = r'''
    ...
    def run_automation(self, k, m, bank, borrower_count, loan_count):
        try:
            ...
            self.root.after(0, lambda msg=str(e): messagebox.showerror("AI Error", msg))
        finally:
...
DASHBOARD = r'''
    ...
    def get_context_from_ui(self):
        try:
            return self.enforce_context_counts({
    ...
    def run_watcher(self):
        if not self.active_case_id or not self.watch_folder or not getattr(self, 'watch_var', None) or not self.watch_var.get():
...
# --- ASSEMBLY ---
out = IMPORTS + CONSTANTS_BLOCK + "\nclass LawApp:\n"
out += HELPERS + NEW_INIT + NEW_SETUP_UI
...
for m in logic_m:
    out += get_body(orig_code, m) + "\n"

out += V5_LOGIC + DASHBOARD
