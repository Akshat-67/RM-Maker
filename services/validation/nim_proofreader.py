    ...
    def proofread(docx_text: str, session_data: dict, ocr_text: str = "", legal_text: str = "", tech_text: str = "") -> list[Discrepancy]:
        ...
        try:
            ...
            prompt = (
                ...
                "5. Compare loan details (Loan Amount, Rate of Interest, Tenure/Period, Loan Account Number (LAN), and EMI details) three-way between the Sanction Letter text (present in the Raw KYC/Sanction PDF text), the form variables (Ground Truth), and the rendered deed draft. (a) Always verify that the form variables match the Sanction Letter for all fields. (b) For the deed draft text: Loan Amount, Interest Rate, and Tenure must always match. (c) Only expect the Loan Account Number (LAN) in the deed draft text if the bank is ICICI (or if it is explicitly referenced in the draft). (d) Only expect EMI details in the deed draft text if the bank format includes EMI clauses. Do not flag LAN or EMI as missing from the deed draft for other banks.\n"
