...
class DevLysToUnicodeConverter:
    ...
    # Compiled Regexes for English checks
    _non_english_chars_regex = re.compile(r'[^A-Za-z0-9\s\.,\-\(\)\/\#\&\:\@]')
    ...
    _clean_word_regex = re.compile(r'^[^A-Za-z0-9]+|[^A-Za-z0-9]+$')

    # Static Sets
    _short_english_words = frozenset(["and", "the", "for", "o", "of", "to", "in", "on", "at", "by", "with", "from", "as", "is", "are", "was", "were", "be"])

    # Compiled Regexes for Matra reordering
    _re_alt_chhoti_i = re.compile(r'Æ((?:[\u0900-\u097F]्)*[\u0900-\u097F])')
    _re_chhoti_i_reorder = re.compile(r'ि((?:[\u0900-\u097F]्)*[\u0900-\u097F])')
    _re_reph_reorder = re.compile(r'((?:[\u0900-\u097F]्)*[\u0900-\u097F][\u093e-\u094d\u0902\u0903]*)Z')
    ...
    def is_likely_english(cls, text):
        ...
        if cls._non_english_chars_regex.search(stripped):
            return False
        ...
        if cls._devlys_specifics_regex.search(stripped):
            return False

        valid_word_found = False
        for w in words:
            # Clean punctuation from ends
            w_clean = cls._clean_word_regex.sub('', w)
            if not w_clean:
                continue
            ...
            if w_clean.islower() and len(w_clean) >= 3 and w_clean in cls._short_english_words:
                continue
            
            return False

        return valid_word_found
    ...
    def devlys_to_unicode_text(cls, text):
        ...
        modified = cls._re_alt_chhoti_i.sub(r'ि\1Z', modified)
        ...
        modified = cls._re_chhoti_i_reorder.sub(r'\1ि', modified)
        ...
        modified = cls._re_reph_reorder.sub(r'र्\1', modified)
