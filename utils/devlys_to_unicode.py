...
class DevLysToUnicodeConverter:
    ...
    _common_short_english = {"and", "the", "for", "o", "of", "to", "in", "on", "at", "by", "with", "from", "as", "is", "are", "was", "were", "be"}
    ...
    def is_likely_english(text):
        ...
        for w in words:
            ...
            w_clean = DevLysToUnicodeConverter._clean_word_regex.sub('', w)
            if not w_clean:
                continue
            if w_clean.isdigit():
                continue
            ...
            if w_clean.islower() and len(w_clean) >= 3 and w_clean in DevLysToUnicodeConverter._common_short_english:
                continue
            
            # If we reach here, it failed all english heuristic checks, return False immediately
            return False

        # If we loop through all words without returning False, it's likely English
        return True
