...
class DevLysToUnicodeConverter:
    ...
    _std_eng_check = re.compile(r'^[A-Za-z0-9\s\.,\-\(\)\/\#\&\:\@]+$')
    ...
    _short_words = frozenset(["and", "the", "for", "o", "of", "to", "in", "on", "at", "by", "with", "from", "as", "is", "are", "was", "were", "be"])
    ...
    def is_likely_english(text):
        ...
        if not DevLysToUnicodeConverter._std_eng_check.match(stripped):
            return False
            
        # Common English terms in legal docs (case-insensitive)
        if DevLysToUnicodeConverter._common_english.search(stripped):
            return True
        ...
        for w in words:
            ...
            if not w_clean:
                continue
            if w_clean.isdigit() or w_clean.isupper():
                continue
            # If Capitalized (e.g. "Name", "Date")
            if len(w_clean) >= 3:
                if w_clean[0].isupper() and w_clean[1:].islower():
                    continue
                # Common short lowercase English words
                if w_clean.islower() and w_clean in DevLysToUnicodeConverter._short_words:
                    continue
