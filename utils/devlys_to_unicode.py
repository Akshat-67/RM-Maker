...
class DevLysToUnicodeConverter:
    ...
    # ⚡ Bolt Optimization: Hoisted compiled regexes and static sets out of the loop
    # to eliminate redundant allocations and re-compilations on every method call.
    ...
    _allowed_chars_regex = re.compile(r'^[A-Za-z0-9\s\.,\-\(\)\/\#\&\:\@]+$')
    _clean_punct_regex = re.compile(r'^[^A-Za-z0-9]+|[^A-Za-z0-9]+$')
    _devlys_specifics = {"Jh", "fo;", "eukst", 'o"kZ', "fHkok", "iq=", "vk;q", "fuoklh", "izFkei{k", "f}rh;i{k"}
    _common_short_words = {"and", "the", "for", "o", "of", "to", "in", "on", "at", "by", "with", "from", "as", "is", "are", "was", "were", "be"}
    ...
    def is_likely_english(text):
        ...
        if not DevLysToUnicodeConverter._allowed_chars_regex.match(stripped):
            return False
            
        # Common English terms in legal docs
        ...
        # ⚡ Bolt Optimization: Using early exit return strategy instead of
        # list accumulation combined with all() to avoid unnecessary O(N) object allocations
        for w in words:
            # Clean punctuation from ends
            w_clean = DevLysToUnicodeConverter._clean_punct_regex.sub('', w)
            if not w_clean:
                continue
            if w_clean.isdigit():
                continue
            ...
            if w_clean.islower() and len(w_clean) >= 3 and w_clean in DevLysToUnicodeConverter._common_short_words:
                continue
            
            # If we reach here, the word didn't match any English heuristics
            return False
