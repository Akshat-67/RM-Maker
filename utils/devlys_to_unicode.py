...
class DevLysToUnicodeConverter:
    ...
    _english_punct_regex = re.compile(r'^[A-Za-z0-9\s\.,\-\(\)\/\#\&\:\@]+$')
    ...
    def is_likely_english(text):
        ...
        if not DevLysToUnicodeConverter._english_punct_regex.match(stripped):
            return False
        ...
        for w in words:
            ...
            w_clean = DevLysToUnicodeConverter._punct_clean_regex.sub('', w)
            if not w_clean:
                continue
