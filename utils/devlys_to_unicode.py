...
class DevLysToUnicodeConverter:
    ...
    _common_short_english_words = {"and", "the", "for", "o", "of", "to", "in", "on", "at", "by", "with", "from", "as", "is", "are", "was", "were", "be"}
    ...
    def is_likely_english(text):
        ...
        for w in words:
            ...
            if w_clean.islower() and len(w_clean) >= 3:
                if w_clean in DevLysToUnicodeConverter._common_short_english_words:
                    continue
            
            # If we reach here, the word failed all English heuristic checks
            return False

        # If we made it through all words without returning False, they are English
        return True
