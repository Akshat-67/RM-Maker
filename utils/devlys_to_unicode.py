...
class DevLysToUnicodeConverter:
    ...
    _short_english_words_set = {"and", "the", "for", "o", "of", "to", "in", "on", "at", "by", "with", "from", "as", "is", "are", "was", "were", "be"}
    _punct_clean_regex = re.compile(r'^[^A-Za-z0-9]+|[^A-Za-z0-9]+$')
    ...
    def is_likely_english(cls, text):
        ...
        if not cls._english_chars_regex.match(stripped):
            return False
        ...
        for x in cls._devlys_specifics:
            if x in stripped:
                return False

        for w in words:
            # Clean punctuation from ends
            w_clean = cls._punct_clean_regex.sub('', w)
            if not w_clean:
                continue
            if w_clean.isdigit():
                continue
            ...
            if w_clean.islower() and len(w_clean) >= 3:
                if w_clean in cls._short_english_words_set:
                    continue
            
            # Early exit: if word fails all checks
            return False

        # If all non-empty words passed English checks
        return True
