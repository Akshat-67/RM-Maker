...
class DevLysToUnicodeConverter:
    # ⚡ Bolt: Hoisting compiled regexes and static sets to class-level to prevent
    # redundant allocations and recompilation overhead in hot text processing loops.
    ...
    _english_short_words = {"and", "the", "for", "o", "of", "to", "in", "on", "at", "by", "with", "from", "as", "is", "are", "was", "were", "be"}
    ...
    def is_likely_english(text):
        ...
        for w in words:
            ...
            if w_clean.islower() and len(w_clean) >= 3:
                if w_clean in DevLysToUnicodeConverter._english_short_words:
                    is_english_words.append(True)
                    continue
