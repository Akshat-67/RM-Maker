...
class DevLysToUnicodeConverter:
    ...
    _re_english_chars = re.compile(r'^[A-Za-z0-9\s\.,\-\(\)\/\#\&\:\@]+$')
    _re_common_english = re.compile(
    ...
    _re_clean_punct = re.compile(r'^[^A-Za-z0-9]+|[^A-Za-z0-9]+$')
    _devlys_specifics_set = {"Jh", "fo;", "eukst", "o\"kZ", "fHkok", "iq=", "vk;q", "fuoklh", "izFkei{k", "f}rh;i{k"}
    _common_short_set = {"and", "the", "for", "o", "of", "to", "in", "on", "at", "by", "with", "from", "as", "is", "are", "was", "were", "be"}

    _re_alt_code = re.compile(r'Æ((?:[\u0900-\u097F]्)*[\u0900-\u097F])')
    _re_chhoti_i = re.compile(r'ि((?:[\u0900-\u097F]्)*[\u0900-\u097F])')
    _re_reph = re.compile(r'((?:[\u0900-\u097F]्)*[\u0900-\u097F][\u093e-\u094d\u0902\u0903]*)Z')
    ...
    def is_likely_english(text):
        ...
        if not DevLysToUnicodeConverter._re_english_chars.match(stripped):
            return False
            
        # Common English terms in legal docs (case-insensitive)
        if DevLysToUnicodeConverter._re_common_english.search(stripped):
            return True
        ...
        for w in words:
            ...
            w_clean = DevLysToUnicodeConverter._re_clean_punct.sub('', w)
            if not w_clean:
                continue
            ...
            if w_clean.islower() and len(w_clean) >= 3 and w_clean in DevLysToUnicodeConverter._common_short_set:
                continue
    ...
    def devlys_to_unicode_text(text):
        ...
        modified = DevLysToUnicodeConverter._re_alt_code.sub(r'ि\1Z', modified)
        ...
        modified = DevLysToUnicodeConverter._re_chhoti_i.sub(r'\1ि', modified)
        ...
        modified = DevLysToUnicodeConverter._re_reph.sub(r'र्\1', modified)
