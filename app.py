...
def smart_merge(old, new, verified_fields, path=""):
    ...
    if isinstance(new, dict):
        ...
        # If it is one of our entity lists: 'ls', 'ps', 'unassigned_aadhars', 'sellers', 'buyers', 'chain'
        # We merge them by unique keys rather than index to prevent overwriting during incremental scans
        if path in ["ls", "ps", "unassigned_aadhars", "sellers", "buyers", "chain",
                    "data.ls", "data.ps", "data.unassigned_aadhars", "data.sellers", "data.buyers", "data.chain"]:
            if "ls" in path: key_field = "n"
            elif "unassigned" in path: key_field = "id"
            elif "sellers" in path or "buyers" in path: key_field = "n"
            elif "chain" in path: key_field = "date"
            else: key_field = "adr"
...
def generate_rm(case_id):
    ...
    try:
        ...
        if doc_type == "SD":
            processor.generate_sd(context, output_filepath, highlight_ai=True, highlight_missing=True, verified_fields=verified_fields)
        else:
